#ifndef MYQUEUEQSTRING_H
#define MYQUEUEQSTRING_H
...
/**
 * @brief MyQueueQString - 用于替代 std::queue<QString> 的自定义队列类
 * 
 * 特点：
 ...
 * - 提供完整的拷贝语义和异常安全
 */
class MyQueueQString {
private:
    QString* m_data;          // 数据数组指针（循环数组）
...
public:
     ...
     */
    explicit MyQueueQString(size_t initialCapacity = 16);
    
    /**
     * @brief 拷贝构造函数
     * @param other 要拷贝的 MyQueueQString 对象
     */
    MyQueueQString(const MyQueueQString& other);
    ...
    /**
     * @brief 赋值运算符
     * @param other 要赋值的 MyQueueQString 对象
     * @return 返回自身的引用
     */
    MyQueueQString& operator=(const MyQueueQString& other);
    
    // ========== 队列操作 ==========
    
    /**
     * @brief 将元素加入队尾
     * @param value 要加入的 QString 值
     */
     ...
     */
    void swap(MyQueueQString& other);
};

#endif // MYQUEUEQSTRING_H
